...
public class MusicPlaybackQueueStore extends SQLiteOpenHelper {
     ...
     */
    public synchronized void saveQueue(final String tableName, @NonNull final ArrayList<Song> queue) {
    ...
    public class SaveQueueAsyncTask extends AsyncTask<List<MediaSessionCompat.QueueItem>,Void,ArrayList<Song>>
    {

        Context context;
        public SaveQueueAsyncTask(Context context)
        {
        ...
        protected ArrayList<Song> doInBackground(List<MediaSessionCompat.QueueItem>[] lists) {
            List<MediaSessionCompat.QueueItem> itemArrayList=lists[0];
            ArrayList<Song>songs=new ArrayList<>();

            for(MediaSessionCompat.QueueItem mediaItem:itemArrayList)
                songs.add(SongLoader.getSong(context, Integer.parseInt(mediaItem.getDescription().getMediaId())));
        ...
        protected void onPostExecute(ArrayList<Song> songs) {
            saveQueue(PLAYING_QUEUE_TABLE_NAME,songs);
        }
    }

    public class SaveOriginalQueueAsyncTask extends AsyncTask<List<MediaSessionCompat.QueueItem>,Void,ArrayList<Song>>
    {

        Context context;
        public SaveOriginalQueueAsyncTask(Context context)
        {
            this.context=context;
        }
        ...
        @Override
        protected void onPostExecute(ArrayList<Song> songs) {
            saveQueue(ORIGINAL_PLAYING_QUEUE_TABLE_NAME,songs);
        }
    }
